...
import { ScrollText, Search, RefreshCw, Monitor, ListVideo, Image as ImageIcon, Send, Trash2, Plus, Edit, Wifi, WifiOff, ChevronLeft, ChevronRight, Download, Siren } from "lucide-react";
...
export default function Logs() {
  ...
  const { data, isLoading, refetch, isFetching } = useQuery<LogsResponse>({
    queryKey: ["logs", page, actionFilter, entityFilter],
    queryFn: async () => {
  ...
  return (
    <div className="space-y-5">
      <PageHeader
        ...
        description={isLoading ? "Carregando..." : `${data?.total ?? 0} registro${(data?.total ?? 0) !== 1 ? "s" : ""} no total`}
        actions={
      ...
      <div className="flex flex-wrap gap-3 items-center">
        <div className="relative flex-1 min-w-[200px] max-w-sm">
          ...
          <Input placeholder="Buscar por nome, ação..." value={search} onChange={e => { setSearch(e.target.value); setPage(1); }} className="pl-9" />
        </div>
        ...
        <div className="flex items-center rounded-md border overflow-hidden text-xs">
          {actions.map(a => (
            <button key={a}
              ...
              className={cn("px-2.5 py-1.5 transition-colors", actionFilter === a ? "bg-primary/15 text-primary font-medium" : "text-muted-foreground hover:bg-muted/40")}
            >
        ...
        <div className="flex items-center rounded-md border overflow-hidden text-xs">
          {entities.map(e => (
            <button key={e}
              ...
              className={cn("px-2.5 py-1.5 transition-colors", entityFilter === e ? "bg-primary/15 text-primary font-medium" : "text-muted-foreground hover:bg-muted/40")}
            >
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b bg-muted/40">
                <th className="px-4 py-3 text-left w-[170px]"><span className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Data/Hora</span></th>
                <th className="px-4 py-3 text-left w-[110px]"><span className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Ação</span></th>
                <th className="px-4 py-3 text-left"><span className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Descrição</span></th>
                <th className="px-4 py-3 text-center w-[110px]"><span className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Status Tela</span></th>
                <th className="px-4 py-3 text-center w-[80px] hidden md:table-cell"><span className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Tipo</span></th>
              </tr>
            </thead>
            <tbody>
              {isLoading ? (
                ...
                <tr>
                  <td colSpan={5} className="text-center py-16 px-4">
                    <div className="flex flex-col items-center">
                      ...
                      <p className="text-muted-foreground text-sm mt-1">Tente mudar os filtros.</p>
                    </div>
                  </td>
                </tr>
              ) : (
                      ...
                      <td className="px-4 py-3">
                        ...
                        {(log.screenId || log.playlistId) && (
                          <div className="flex items-center gap-2 mt-0.5">
                            {log.screenId && <span className="text-[10px] text-muted-foreground">Tela #{log.screenId}</span>}
                            {log.playlistId && <span className="text-[10px] text-muted-foreground">Playlist #{log.playlistId}</span>}
                          </div>
                        )}
                      </td>

                      {/* Status da tela na hora */}
