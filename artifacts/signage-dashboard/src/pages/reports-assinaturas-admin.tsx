...
function lastPayment(o: Operator) {
  const pays = Array.isArray(o.payments) ? o.payments : [];
  return pays.filter(p => p.paidAt).sort((a, b) => (b.paidAt ?? "").localeCompare(a.paidAt ?? ""))[0];
}
function printReport(rows: Operator[]) {
  const logo = `${window.location.origin}/logo-rpshow.png`;
  const now = new Date().toLocaleString("pt-BR", { timeZone: "America/Sao_Paulo" });
  const mrr = rows.filter(r => r.subscriptionStatus === "active").reduce((s, r) => s + num(r.monthlyAmount), 0);
  const body = rows.map(r => {
    const lastPay = lastPayment(r);
    return `<tr><td>${r.name}</td><td class="center">${STATUS_META[r.subscriptionStatus]?.label ?? r.subscriptionStatus}</td><td class="center">${r.screenCount}</td><td class="mono">${brl(num(r.pricePerScreen))}</td><td class="mono">${brl(num(r.monthlyAmount))}</td><td class="mono">${fmtDate(r.trialEndsAt)}</td><td class="mono">${fmtDate(lastPay?.paidAt ?? null)}</td></tr>`;
  }).join("");
...
export default function ReportsAssinaturasAdmin() {
  ...
  // /api/admin/operators NÃO traz payments — causa crash em o.payments.filter.
  // /api/admin/financial tem o mesmo shape + payments[].
  const { data, isLoading, isError, error, refetch } = useQuery<Operator[]>({
    queryKey: ["admin-report-assinaturas"],
    queryFn: async () => {
      const r = await fetch("/api/admin/financial", { credentials: "include" });
      const json = await r.json().catch(() => null);
      if (!r.ok) throw new Error((json as { error?: string } | null)?.error || `HTTP ${r.status}`);
      if (!Array.isArray(json)) return [];
      return json.map((o: Operator) => ({
        ...o,
        payments: Array.isArray(o.payments) ? o.payments : [],
        pricePerScreen: o.pricePerScreen ?? "0",
        monthlyAmount: o.monthlyAmount ?? "0",
        screenCount: Number(o.screenCount) || 0,
      }));
    },
  });
  ...
  const mrr          = operators.filter(o => o.subscriptionStatus === "active").reduce((s, o) => s + num(o.monthlyAmount), 0);
    ...
    <div className="space-y-5">
      ...
      {isError && (
        <div className="rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700 flex items-center justify-between gap-3">
          <span>Erro ao carregar assinaturas: {(error as Error)?.message || "tente novamente"}</span>
          <Button variant="outline" size="sm" className="h-7 text-xs" onClick={() => refetch()}>Tentar de novo</Button>
        </div>
      )}
      ...
      <div className="bg-card border rounded-xl overflow-hidden">
                ...
                {filtered.map(o => {
                  ...
                  return (
                    <tr key={o.id} className="hover:bg-muted/30 transition-colors">
                      ...
                      <td className="px-4 py-2.5 font-mono text-[11px]">{brl(num(o.pricePerScreen))}</td>
                      <td className="px-4 py-2.5 font-mono text-[11px] font-bold text-primary">{brl(num(o.monthlyAmount))}</td>
                      <td className="px-4 py-2.5">
                        {o.trialEndsAt ? (
        ...
        {!isLoading && filtered.length > 0 && (
          <div className="px-4 py-3 border-t bg-muted/30 flex items-center gap-6 text-xs">
            ...
            <span className="font-bold text-emerald-600 text-base">{brl(filtered.filter(o => o.subscriptionStatus === "active").reduce((s, o) => s + num(o.monthlyAmount), 0))}</span>
          </div>
        )}
      </div>
    </div>
